...
public class IntList {
	...
	public IntList(final int[] source) {
		count = source.length;
		data = new int[count];
	...
	public int removeIndex(final int index) {
		final int bye = data[index];
		PApplet.println(data);
		final int[] outgoing = new int[count - 1];
	...
	public int index(final int what) {
		/*
		 * if (indexCache != null) { try { return indexCache.get(what); } catch (Exception e) { //
		 * not there return -1; } }
		 */
	...
	public boolean contains(final int what) {
		// if (indexCache == null) {
		// cacheIndices();
		// }
		// return index(what) != -1;
	...
	public void mul(final int amt) {
		for (int i = 0; i < count; i++) {
			data[i] *= amt;
		}
	}

	public void div(final int amt) {
		for (int i = 0; i < count; i++) {
			data[i] /= amt;
		}
	}
