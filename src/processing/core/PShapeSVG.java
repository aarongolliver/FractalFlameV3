...
/*
 * Part of the Processing project - http://processing.org Copyright (c) 2006-11 Ben Fry and Casey
 * Reas Copyright (c) 2004-06 Michael Chang This library is free software; you can redistribute it
 * and/or modify it under the terms of the GNU Lesser General Public License version 2.1 as
 * published by the Free Software Foundation. This library is distributed in the hope that it will
 * be useful, but WITHOUT ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or
 * FITNESS FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more details. You
 * should have received a copy of the GNU Lesser General Public License along with this library; if
...
/**
 * SVG stands for Scalable Vector Graphics, a portable graphics format. It is a vector format so it
 * allows for infinite resolution and relatively small file sizes. Most modern media software can
 * view SVG files, including Adobe products, Firefox, etc. Illustrator and Inkscape can edit SVG
 * files.
 * <p>
 * We have no intention of turning this into a full-featured SVG library. The goal of this project
 * is a basic shape importer that is small enough to be included with applets, meaning that its
 * download size should be in the neighborhood of 25-30k. Starting with release 0149, this library
 * has been incorporated into the core via the loadShape() command, because vector shape data is
 * just as important as the image data from loadImage().
 * <p>
 * For more sophisticated import/export, consider the <A
 * HREF="http://xmlgraphics.apache.org/batik/">Batik</A> library from the Apache Software
 * Foundation. Future improvements to this library may focus on this properly supporting a specific
 * subset of SVG, for instance the simpler SVG profiles known as <A
 * HREF="http://www.w3.org/TR/SVGMobile/">SVG Tiny or Basic</A>, although we still would not support
 ...
 * </PRE>
 * 
 * This code is based on the Candy library written by Michael Chang, which was later revised and
 * expanded for use as a Processing core library by Ben Fry. Thanks to Ricard Marxer Pinon for help
 * with better Inkscape support in 0154.
 * <p>
 * <hr noshade>
 * <p>
 ...
 * </UL>
 * Some SVG objects and features may not yet be supported. Here is a partial list of non-included
 * features
 * <UL>
...
public class PShapeSVG extends PShape {
	...
	public PShapeSVG(final XML svg) {
		...
		final String viewBoxStr = svg.getString("viewBox");
		if (viewBoxStr != null) {
	...
	protected PShapeSVG(final PShapeSVG parent, final XML properties, final boolean parseKids) {
		...
		if (name != null) {
			while (true) {
				final String[] m = PApplet.match(name, "_x([A-Za-z0-9]{2})_");
				if (m == null) {
					break;
				}
		...
		final String displayStr = properties.getString("display", "inline");
	 ...
	 */
	protected PShape parseChild(final XML elem) {
	...
	protected void parsePath() {
		...
		while (i < pathTokens.length) {
			...
			// C - curve to (absolute)
			case 'C': {
				final double ctrlX1 = PApplet.parseFloat(pathTokens[i + 1]);
				final double ctrlY1 = PApplet.parseFloat(pathTokens[i + 2]);
				final double ctrlX2 = PApplet.parseFloat(pathTokens[i + 3]);
				final double ctrlY2 = PApplet.parseFloat(pathTokens[i + 4]);
				final double endX = PApplet.parseFloat(pathTokens[i + 5]);
				final double endY = PApplet.parseFloat(pathTokens[i + 6]);
			...
			// c - curve to (relative)
			case 'c': {
				final double ctrlX1 = cx + PApplet.parseFloat(pathTokens[i + 1]);
				final double ctrlY1 = cy + PApplet.parseFloat(pathTokens[i + 2]);
				final double ctrlX2 = cx + PApplet.parseFloat(pathTokens[i + 3]);
				final double ctrlY2 = cy + PApplet.parseFloat(pathTokens[i + 4]);
				final double endX = cx + PApplet.parseFloat(pathTokens[i + 5]);
				final double endY = cy + PApplet.parseFloat(pathTokens[i + 6]);
				parsePathCurveto(ctrlX1, ctrlY1, ctrlX2, ctrlY2, endX, endY);
				cx = endX;
				cy = endY;
				i += 7;
			...
			case 'S': {
				...
				final double ctrlX2 = PApplet.parseFloat(pathTokens[i + 1]);
				final double ctrlY2 = PApplet.parseFloat(pathTokens[i + 2]);
				final double endX = PApplet.parseFloat(pathTokens[i + 3]);
				final double endY = PApplet.parseFloat(pathTokens[i + 4]);
			...
			case 's': {
				...
				final double ctrlX2 = cx + PApplet.parseFloat(pathTokens[i + 1]);
				final double ctrlY2 = cy + PApplet.parseFloat(pathTokens[i + 2]);
				final double endX = cx + PApplet.parseFloat(pathTokens[i + 3]);
				final double endY = cy + PApplet.parseFloat(pathTokens[i + 4]);
				parsePathCurveto(ctrlX, ctrlY, ctrlX2, ctrlY2, endX, endY);
				cx = endX;
				cy = endY;
				i += 5;
			...
			case 'T': {
				...
				final double endX = PApplet.parseFloat(pathTokens[i + 1]);
				final double endY = PApplet.parseFloat(pathTokens[i + 2]);
				// parsePathQuadto(cx, cy, ctrlX, ctrlY, endX, endY);
				parsePathQuadto(ctrlX, ctrlY, endX, endY);
				cx = endX;
				cy = endY;
			...
			case 't': {
				...
				final double endX = cx + PApplet.parseFloat(pathTokens[i + 1]);
				final double endY = cy + PApplet.parseFloat(pathTokens[i + 2]);
				// parsePathQuadto(cx, cy, ctrlX, ctrlY, endX, endY);
				parsePathQuadto(ctrlX, ctrlY, endX, endY);
				cx = endX;
				cy = endY;
			...
			default:
				final String parsed = PApplet.join(PApplet.subset(pathTokens, 0, i), ",");
				final String unparsed = PApplet.join(PApplet.subset(pathTokens, i), ",");
				System.err.println("parsed: " + parsed);
				System.err.println("unparsed: " + unparsed);
				if (pathTokens[i].equals("a") || pathTokens[i].equals("A")) {
					final String msg = "Sorry, elliptical arc support for SVG files "
	...
	private void parsePathVertex(final double x, final double y) {
		if (vertexCount == vertices.length) {
			// vertices = (double[][]) PApplet.expand(vertices);
			final double[][] temp = new double[vertexCount << 1][2];
			System.arraycopy(vertices, 0, temp, 0, vertexCount);
			vertices = temp;
		}
	...
	protected void parseColors(final XML properties) {
		if (properties.hasAttribute("opacity")) {
			final String opacityText = properties.getString("opacity");
			setOpacity(opacityText);
		}

		if (properties.hasAttribute("stroke")) {
			final String strokeText = properties.getString("stroke");
			setColor(strokeText, false);
		}

		if (properties.hasAttribute("stroke-opacity")) {
			final String strokeOpacityText = properties.getString("stroke-opacity");
		...
		if (properties.hasAttribute("stroke-linejoin")) {
			final String linejoin = properties.getString("stroke-linejoin");
			setStrokeJoin(linejoin);
		}

		if (properties.hasAttribute("stroke-linecap")) {
			final String linecap = properties.getString("stroke-linecap");
		...
		if (properties.hasAttribute("fill")) {
			final String fillText = properties.getString("fill");
			setColor(fillText, true);
		}

		if (properties.hasAttribute("fill-opacity")) {
			final String fillOpacityText = properties.getString("fill-opacity");
			setFillOpacity(fillOpacityText);
		}

		if (properties.hasAttribute("style")) {
			final String styleText = properties.getString("style");
			final String[] styleTokens = PApplet.splitTokens(styleText, ";");

			// PApplet.println(styleTokens);
			for (final String styleToken : styleTokens) {
				final String[] tokens = PApplet.splitTokens(styleToken, ":");
	...
	void setColor(String colorText, final boolean isFill) {
		final int opacityMask = fillColor & 0xFF000000;
		...
		if (colorText.equals("none")) {
			...
			final Object object = findChild(name);
	...
	static protected int parseRGB(final String what) {
		final int leftParen = what.indexOf('(') + 1;
		final int rightParen = what.indexOf(')');
		final String sub = what.substring(leftParen, rightParen);
		final int[] values = PApplet.parseInt(PApplet.splitTokens(sub, ", "));
		return (values[0] << 16) | (values[1] << 8) | (values[2]);
	}

	static protected HashMap<String, String> parseStyleAttributes(final String style) {
	...
	/**
	 * Parse a size that may have a suffix for its units. Ignoring cases where this could also be a
	 * percentage. The <A HREF="http://www.w3.org/TR/SVG/coords.html#Units">units</A> spec:
	 * <UL>
	 ...
	 * </UL>
	 */
	static protected double parseUnitSize(final String text) {
	...
	class LinearGradient extends Gradient {
		...
		public LinearGradient(final PShapeSVG parent, final XML properties) {
			...
			if (transformStr != null) {
				...
				final Point2D t2 = transform.transform(new Point2D.Double(x2, y2), null);
	...
	class RadialGradient extends Gradient {
		...
		public RadialGradient(final PShapeSVG parent, final XML properties) {
			...
			if (transformStr != null) {
				...
				final Point2D t2 = transform.transform(new Point2D.Double(cx + r, cy), null);
	...
	class LinearGradientPaint implements Paint {
		...
		public LinearGradientPaint(final double x1, final double y1, final double x2, final double y2,
		        final double[] offset, final int[] color, final int count, final double opacity) {
			this.x1 = x1;
			this.y1 = y1;
			this.x2 = x2;
			this.y2 = y2;
		...
		public PaintContext createContext(final ColorModel cm, final Rectangle deviceBounds,
			...
			final Point2D t1 = xform.transform(new Point2D.Double(x1, y1), null);
			final Point2D t2 = xform.transform(new Point2D.Double(x2, y2), null);
			return new LinearGradientContext(t1.getX(), t1.getY(), t2.getX(), t2.getY());
		}
		...
		public class LinearGradientContext implements PaintContext {
			...
			public LinearGradientContext(final double tx1, final double ty1, final double tx2, final double ty2) {
				this.tx1 = tx1;
				this.ty1 = ty1;
				this.tx2 = tx2;
				this.ty2 = ty2;
			...
			public Raster getRaster(final int x, final int y, final int w, final int h) {
				...
				final int span = (int) PApplet.dist(tx1, ty1, tx2, ty2) * ACCURACY;
				if (span <= 0) {
					...
					for (int i = 1; i < count; i++) {
						final int c0 = color[i - 1];
						final int c1 = color[i];
						final int last = (int) (offset[i] * (span - 1));
						// System.out.println("last is " + last);
						for (int j = prev; j <= last; j++) {
							final double btwn = PApplet.norm(j, prev, last);
	...
	class RadialGradientPaint implements Paint {
		...
		public RadialGradientPaint(final double cx, final double cy, final double radius, final double[] offset,
		        final int[] color, final int count, final double opacity) {
			this.cx = cx;
			this.cy = cy;
			this.radius = radius;
			this.offset = offset;
		...
		public class RadialGradientContext implements PaintContext {
			...
			public Raster getRaster(final int x, final int y, final int w, final int h) {
				...
				final int span = (int) radius * ACCURACY;
				final int[][] interp = new int[span][4];
				int prev = 0;
				for (int i = 1; i < count; i++) {
					final int c0 = color[i - 1];
				...
				for (int j = 0; j < h; j++) {
					for (int i = 0; i < w; i++) {
						final double distance = PApplet.dist(cx, cy, x + i, y + j);
						final int which = PApplet.min((int) (distance * ACCURACY), interp.length - 1);
	...
	protected Paint calcGradientPaint(final Gradient gradient) {
		if (gradient instanceof LinearGradient) {
			...
			return new LinearGradientPaint(grad.x1, grad.y1, grad.x2, grad.y2, grad.offset, grad.color, grad.count,
			        opacity);

		} else if (gradient instanceof RadialGradient) {
			final RadialGradient grad = (RadialGradient) gradient;
	...
	public class Font extends PShapeSVG {
		...
		public void drawString(final PGraphics g, final String str, final double x, final double y, final double size) {
			// 1) scale by the 1.0/unitsPerEm
			// 2) scale up by a font size
			g.pushMatrix();
			final double s = size / face.unitsPerEm;
			// System.out.println("scale is " + s);
			// swap y coord at the same time, since fonts have y=0 at baseline
			g.translate(x, y);
			g.scale(s, -s);
			final char[] c = str.toCharArray();
			for (final char element2 : c) {
		...
		public void drawChar(final PGraphics g, final char c, final double x, final double y, final double size) {
			g.pushMatrix();
			final double s = size / face.unitsPerEm;
			g.translate(x, y);
			g.scale(s, -s);
			final FontGlyph fg = unicodeGlyphs.get(new Character(c));
			if (fg != null) {
				g.shape(fg);
			}
			g.popMatrix();
		...
		public double textWidth(final String str, final double size) {
			double w = 0;
			final char[] c = str.toCharArray();
			for (final char element2 : c) {
				// call draw on each char (pulling it w/ the unicode table)
	...
	class FontFace extends PShapeSVG {
		...
		public FontFace(final PShapeSVG parent, final XML properties) {
			super(parent, properties, true);
